...
| MCP tools | Graph memory: `set_active_topic`, `query_context`, `query_graph`, `get_node`, `emit_node`, `link`; source discovery: `index_codebase`, `search_source`, `get_index_status`, `clear_index` |
| CLI subcommands | `init`, `show`, `correct`, `deprecate`, `validate`, `rollup`, `scan`, `context`, `search-source`, `index-status`, `clear-index` |
...
- Use the lifecycle in `AGENTS.md` for any codemap-source task: `set_active_topic` → `query_context` (or `query_graph` + optional `search_source`) → emit findings → link relationships.
